...
describe("Change Password Route", () => {
    ...
    describe("GET /changepassword", () => {
        it("should render change password page when no code is provided", async () => {
            const response = await request(app).get("/changepassword").expect(200);
        ...
        it("should render error page when invalid code is provided", async () => {
            const response = await request(app).get(`/changepassword?code=invalid-secret&email=${mockEmail}`).expect(200);
    ...
    describe("POST /changepassword", () => {
        it("should send password reset email when email is provided", async () => {
            const response = await request(app).post("/changepassword").send({ email: mockEmail }).expect(302);

            expect(sendMail).toHaveBeenCalledWith(mockEmail, "Formbar Password Change", expect.stringContaining(mockSecret));
        });
