                    ...
                    try {
                        ...
                        database.get("SELECT * FROM custom_polls WHERE id=?", pollId, (err, poll) => {
                            try {
                                ...
                                database.get("SELECT * FROM shared_polls WHERE pollId=? AND userId=?", [pollId, user.id], (err, sharePoll) => {
                                    try {
                                        if (err) throw err;
                                        ...
                                        database.run("INSERT INTO shared_polls (pollId, userId) VALUES (?, ?)", [pollId, user.id], async (err) => {
                                            try {
                                                if (err) throw err;
            ...
            try {
                database.get("SELECT * FROM shared_polls WHERE pollId=? AND userId=?", [pollId, userId], (err, pollShare) => {
                    try {
                        ...
                        database.run("DELETE FROM shared_polls WHERE pollId=? AND userId=?", [pollId, userId], (err) => {
                            try {
                                if (err) throw err;
            ...
            try {
                database.get("SELECT * FROM class_polls WHERE pollId=? AND classId=?", [pollId, classId], (err, pollShare) => {
                    try {
                        ...
                        database.run("DELETE FROM class_polls WHERE pollId=? AND classId=?", [pollId, classId], (err) => {
                            try {
                                if (err) throw err;
                    ...
                    try {
                        ...
                        database.get("SELECT * FROM custom_polls WHERE id=?", pollId, (err, poll) => {
                            try {
                                ...
                                database.get("SELECT * FROM class_polls WHERE pollId=? AND classId=?", [pollId, classroom.id], (err, sharePoll) => {
                                    try {
                                        if (err) throw err;
