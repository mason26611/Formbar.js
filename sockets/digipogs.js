...
module.exports = {
    run(socket) {
        ...
        onSocketEvent(socket, "awardDigipogs", hasClassScope(SCOPES.CLASS.DIGIPOGS.AWARD), async (socketContext, awardData) => {
            const result = await awardDigipogs(awardData, socketContext.session);
