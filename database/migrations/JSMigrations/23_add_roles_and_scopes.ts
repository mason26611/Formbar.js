...
const { ROLES, ROLE_NAMES } = require("@modules/roles") as {
 ...
 */
function buildScopesJson(roleName: string): string {
...
module.exports = {
    async run(database: sqlite3.Database): Promise<void> {
        ...
        const existingRoles = await dbGetAll<RoleNameRow>("SELECT name FROM roles WHERE classId IS NULL", [], database);
        ...
        const usersColumns = await dbGetAll<PragmaColumnInfo>("PRAGMA table_info(users)", [], database);
        ...
        const classusersColumns = await dbGetAll<PragmaColumnInfo>("PRAGMA table_info(classusers)", [], database);
